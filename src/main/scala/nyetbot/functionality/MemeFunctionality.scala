...
object MemeFunctionality:
    def apply(service: MemeService)(using TelegramClient[IO]): MemeFunctionality =
        new MemeFunctionality with Discard[IO]:
            def triggerMemeScenario: Scenario[IO, Unit] =
                for
                    message       <- Scenario.expect(textMessage)
                    maybeResponses = service.getMemeResponse(message.text)
                    _             <- Scenario.eval(
                                       memeSendingAction(maybeResponses, message.chat, message.messageId)
                                     )
                yield ()
            ...
            def addMemeScenario: Scenario[IO, Unit] =
                for
                    ...
                    chance         <-
                        Scenario.eval(parseChance(chanceString)).handleErrorWith { _ =>
