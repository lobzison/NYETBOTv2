...
trait SwearFunctionality:
    ...
    def scenarios: List[Scenario[IO, Unit]]
    def getOptionalSwear: IO[Option[String]]
    def sendOptionalSwear(msg: TelegramMessage): IO[Unit]

object SwearFunctionality:
    def apply(service: SwearService)(using TelegramClient[IO]): SwearFunctionality =
        new SwearFunctionality with Discard[IO]:
            ...
            override def getOptionalSwear: IO[Option[String]] =
                service.getSwear.map(_.map(_.value))

            override def sendOptionalSwear(msg: TelegramMessage): IO[Unit] =
                for
                ...
                for
                    ...
                    _        <- res.fold(
                                  ...
                                  swears =>
                                      Scenario.eval(
                                        chat.send(
                                          textContent(swears).copy(parseMode = Some(ParseMode.HTML))
                                        )
                                      )
                                )
                yield ()
                                      ...
                                      rawChance =>
                                          Chance
                                              ...
                                              .fold(
                                                _ =>
                                                    Scenario
                                                        .eval(chat.send("Chance must be a positive integer")),
                                                chance =>
                                                    Scenario.eval(
                                                      service.addSwearGroup(chance) >>
                                                          chat.send("Swear group added")
                                                    )
                                              )
                                    )
                yield ()
            ...
            def deleteSwear: Scenario[IO, Unit] =
                for
                    ...
                    _             <- swearIdMaybe.fold(
                                       ...
                                       swearIdInt =>
                                           Scenario.eval(
                                             service.deleteSwear(SwearId(swearIdInt)) >>
                                                 chat.send("Swear deleted")
                                           )
                                     )
                yield ()
