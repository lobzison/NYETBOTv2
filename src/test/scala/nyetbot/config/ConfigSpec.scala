...
class ConfigSpec extends FunSuite:
    ...
    test("application.conf exposes all nyetbot tunables with expected defaults") {
        ...
        assertEquals(root.ollama.classifyRegister.model, "gemma4:e4b")
        assertEquals(root.ollama.classifyRegister.temperature, 0.2)
        assertEquals(root.ollama.classifyRegister.numPredict, 6)
        assertEquals(root.ollama.classifyRegister.numCtx, 8192)
        assertEquals(root.ollama.classifyRegister.think, false)
