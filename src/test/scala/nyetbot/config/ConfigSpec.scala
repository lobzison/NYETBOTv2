...
class ConfigSpec extends FunSuite:
    ...
    test("application.conf exposes all nyetbot tunables with expected defaults") {
        ...
        assertEquals(root.ollama.summarizeThread.model, "gemma4:e4b")
        assertEquals(root.ollama.summarizeThread.temperature, 0.2)
        assertEquals(root.ollama.summarizeThread.numPredict, 160)
        assertEquals(root.ollama.summarizeThread.numCtx, 8192)
        assertEquals(root.ollama.summarizeThread.think, false)
        assertEquals(root.ollama.classifyIntent.model, "gemma4:e4b")
        assertEquals(root.ollama.classifyIntent.temperature, 0.2)
        assertEquals(root.ollama.classifyIntent.numPredict, 4)
        assertEquals(root.ollama.classifyIntent.numCtx, 8192)
        assertEquals(root.ollama.classifyIntent.think, false)
        assertEquals(root.ollama.summarizeUser.model, "gemma4:e4b")
        assertEquals(root.ollama.summarizeUser.temperature, 0.2)
        assertEquals(root.ollama.summarizeUser.summaryNumPredict, 256)
        assertEquals(root.ollama.summarizeUser.rewriteNumPredict, 200)
        assertEquals(root.ollama.summarizeUser.numCtx, 8192)
        assertEquals(root.ollama.summarizeUser.think, false)
