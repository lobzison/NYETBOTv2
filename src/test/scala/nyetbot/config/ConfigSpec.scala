...
class ConfigSpec extends FunSuite:
    ...
    test("application.conf exposes all nyetbot tunables with expected defaults") {
        ...
        assertEquals(root.ollama.replyModel, "NYETBOTv1")
        assertEquals(root.ollama.utilityModel, "gemma4:e4b")
        assertEquals(root.ollama.replyTemperature, 0.85)
        assertEquals(root.ollama.utilityTemperature, 0.2)
        assertEquals(root.ollama.replyNumPredict, 512)
        ...
        assertEquals(root.ollama.numCtx, 8192)
        assertEquals(root.ollama.think, false)
